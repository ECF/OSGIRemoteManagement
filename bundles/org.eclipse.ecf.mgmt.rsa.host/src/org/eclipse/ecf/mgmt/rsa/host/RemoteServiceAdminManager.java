...
public class RemoteServiceAdminManager extends AbstractManager implements
	...
	protected void activate(BundleContext context) throws Exception {
		...
		adminEvents = Collections
				.synchronizedList(new ArrayList<RemoteServiceAdmin.RemoteServiceAdminEvent>());
	}
	...
	protected RemoteServiceAdminEventMTO createEventMTO(
			RemoteServiceAdmin.RemoteServiceAdminEvent e) {
		EndpointDescription ed = e.getEndpointDescription();
		return new RemoteServiceAdminEventMTO(e.getType(), e.getSource()
				.getBundleId(), e.getContainerID(), (ed == null) ? 0L
				: ed.getRemoteServiceId(), (ed == null) ? 0L
				: ed.getServiceId(), (ed == null) ? null : ed.getProperties(),
				e.getException());
	}

	protected ExportReferenceMTO createExportReferenceMTO(EndpointDescription ed) {
		return new ExportReferenceMTO(ed.getContainerID(),
				ed.getRemoteServiceId(), ed.getServiceId(), ed.getProperties());
	}

	protected ImportReferenceMTO createImportReferenceMTO(EndpointDescription ed) {
		return new ImportReferenceMTO(ed.getContainerID(),
				ed.getRemoteServiceId(), ed.getServiceId(), ed.getProperties());
	}

	protected ExportRegistrationMTO createExportRegistrationMTO(
			ExportRegistration er) {
		Throwable t = er.getException();
		if (t != null)
			return new ExportRegistrationMTO(t);
		EndpointDescription ed = (EndpointDescription) er.getExportReference()
				.getExportedEndpoint();
		return new ExportRegistrationMTO(ed.getConnectTargetID(),
				ed.getServiceId(), ed.getServiceId(), ed.getProperties());
	}

	protected ImportRegistrationMTO createImportRegistrationMTO(
			ImportRegistration ir) {
		Throwable t = ir.getException();
		if (t != null)
			return new ImportRegistrationMTO(t);
		EndpointDescription ed = (EndpointDescription) ir.getImportReference()
				.getImportedEndpoint();
		return new ImportRegistrationMTO(ed.getConnectTargetID(),
				ed.getServiceId(), ed.getServiceId(), ed.getProperties());
	}
	...
	protected List<RemoteServiceAdminEvent> getEvents(boolean export) {
		return adminEvents
				.stream()
				.filter(e -> {
					return (e.getType() == RemoteServiceAdminEvent.EXPORT_REGISTRATION
							&& e.getException() == null && ((export && e
							.getExportReference() != null) || (!export && e
	...
	public RemoteServiceAdminEventMTO[] getRemoteServiceAdminEvents(
		...
		List<RemoteServiceAdminEventMTO> results = events.stream()
				.filter(e -> {
					if (typeFilters == null)
						return false;
					else {
	...
	public ExportReferenceMTO[] getExportedServices() {
		List<ExportReferenceMTO> results = getEvents(true)
				.stream()
				.map(e -> {
					return createExportReferenceMTO((EndpointDescription) e
	...
	public ImportReferenceMTO[] getImportedEndpoints() {
		List<ImportReferenceMTO> results = getEvents(false)
				.stream()
				.map(e -> {
					return createImportReferenceMTO((EndpointDescription) e
							.getImportReference().getImportedEndpoint());
				}).collect(Collectors.toList());
		return results.toArray(new ImportReferenceMTO[results.size()]);
	}

	@Override
	public ExportRegistrationMTO[] exportService(
		...
		Collection<ExportRegistration> ers = remoteServiceAdmin.exportService(
				sr, overridingProperties);
	...
	protected org.eclipse.ecf.osgi.services.remoteserviceadmin.RemoteServiceAdmin.ExportRegistration findExportRegistration(
		...
		List<org.eclipse.ecf.osgi.services.remoteserviceadmin.RemoteServiceAdmin.ExportRegistration> results = select(
				remoteServiceAdmin.getExportedRegistrations(),
				er -> {
					ExportReference exRef = (ExportReference) er
							.getExportReference();
					if (exRef == null)
						return false;
	...
	protected org.eclipse.ecf.osgi.services.remoteserviceadmin.RemoteServiceAdmin.ImportRegistration findImportRegistration(
		...
		List<org.eclipse.ecf.osgi.services.remoteserviceadmin.RemoteServiceAdmin.ImportRegistration> results = select(
				remoteServiceAdmin.getImportedRegistrations(),
				ir -> {
					ImportReference iRef = (ImportReference) ir
							.getImportReference();
					if (iRef == null)
						return false;
	...
	@Override
	public EndpointDescriptionMTO updateExport(
			ExportReferenceMTO exportReference, Map<String, ?> properties) {
		@SuppressWarnings("rawtypes")
		ServiceReference sr = findServiceReference(exportReference
				.getExportedService());
		if (sr == null)
			return null;
		...
		if (exportRegistration == null)
			return null;
		EndpointDescription updated = (EndpointDescription) exportRegistration
				.update(properties);
		if (updated == null)
			return null;
	...
	@Override
	public ImportRegistrationMTO importService(
			EndpointDescriptionMTO endpointDescription) {
		ImportRegistration ir = remoteServiceAdmin
				.importService(new EndpointDescription(endpointDescription
						.getProperties()));
	...
	public boolean updateImport(EndpointDescriptionMTO endpoint) {
		EndpointDescription updateEd = new EndpointDescription(
	...
	public boolean closeImport(ImportReferenceMTO importReference) {
		@SuppressWarnings("rawtypes")
		ServiceReference sr = findServiceReference(importReference
				.getImportedService());
		if (sr == null)
			return false;
